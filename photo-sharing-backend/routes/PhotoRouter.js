...
router.post(
  "/posts/new",
  authMiddleWare,
  parseFileMiddleWare.single("file"),
  async (req, res) => {
    try {
      if (!req.file) {
...
router.get("/file/:fileName", (req, res) => {
  ...
  const pathUrl = path.join(__dirname, "..", "images", fileName);
  console.log("path: ", pathUrl);
  res.sendFile(pathUrl, (err) => {
    if (err) {
      console.log("err read file: ", err);
      res.status(500).send({ msg: "error read file" });
    }
  });
});
